...
well.df <- well.df %>%
  group_by(col, row) %>%
  mutate(activity50 = day[which(norm.activity <= 50)[1]])

well.df <- well.df %>%
  group_by(col, row) %>%
  mutate(activity10 = day[which(norm.activity <= 10)[1]])
well.df <- ungroup(well.df)

for (i in 1:nrow(well.df)) {
  if (is.na(well.df$activity0[i]) == TRUE & !(is.na(well.df$norm.activity[i])) == TRUE) {
    well.df$activity0[i] <- 25
  }
}

activity50 <- well.df %>%
  select(strain, activity50)

activity10 <- well.df %>%
  select(strain, activity10)

activity0 <- well.df %>%
  select(strain, activity0)

activity50$pheno <- activity50$activity50
x <- activity50 %>% select(c(strain, pheno))
H2.fun(x)

activity10$pheno <- activity10$activity10
x <- activity10 %>% select(c(strain, pheno))
H2.fun(x)

activity0$pheno <- activity0$activity0
x <- activity0 %>% select(c(strain, pheno))
H2.fun(x)
...
ggplot(well_twos.df) + geom_point(aes(x = day, y = norm.activity)) + geom_line(aes(x = day, y = yvalue, color = "red")) + facet_grid(grouping ~ col, labeller = label_strains) + ylim(0, 100) + presentation + guides(color = FALSE)
...
ggplot(well_threes.df) + geom_point(aes(x = day, y = norm.activity)) + geom_line(aes(x = day, y = yvalue, color = "red")) + facet_grid(grouping ~ col, labeller = label_strains) + ylim(0, 100) + presentation + guides(color = FALSE)
...
ggplot(well_fours.df) + geom_point(aes(x = day, y = norm.activity)) + geom_line(aes(x = day, y = yvalue, color = "red")) + facet_grid(grouping ~ col, labeller = label_strains) + ylim(0, 100) + presentation + guides(color = FALSE)
...
ggplot(well_eights.df) + geom_point(aes(x = day, y = norm.activity)) + geom_line(aes(x = day, y = yvalue, color = "red")) + facet_wrap( ~ uniqueStrain) + ylim(0, 100) + presentation + guides(color = FALSE)
...
ggplot(strain.df) + geom_point(aes(x = day, y = norm.activity)) + geom_line(aes(x = day, y = yvalue, color = "red")) + facet_wrap( ~ strain) + ylim(0, 100) + presentation + guides(color = FALSE)

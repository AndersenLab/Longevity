...
well_params.df <- data.frame(strain = rep(strains, each = length(rows)), col = rep(1:12, each = length(rows)), row = rep(rows, nStrains), Aparam = rep(0, nStrains * replicates), Gparam = rep(0, nStrains * replicates))
...
for (x in 1:12) {
  for (y in rows) {
    if (!(is.na(well.df$norm.activity[well.df$row == y & well.df$col == x]))[1] == TRUE) {
      temp <- nlm(well_gompertz, c(0.004, 0.14), well.df, y, x)[["estimate"]]
      well_params.df$Aparam[l] <- temp[1]
      well_params.df$Gparam[l] <- temp[2]
    }
    l = l + 1
  }
}
...
for (x in 1:12) {
  for (y in rows) {
    well_survivals.df[l] <- draw_gompertz(well_params.df$Aparam[well_params.df$col == x & well_params.df$row == y], well_params.df$Gparam[well_params.df$col == x & well_params.df$row == y])
    l <- l + 1
  }
}
...
well_twos_params.df <- data.frame(strain = rep(strains, each = 4), col = rep(1:12, each = 4), grouping = rep(1:4, nStrains), Aparam = rep(0, nStrains * 4), Gparam = rep(0, nStrains * 4))
...
for (x in 1:12) {
  for (y in 1:4) {
    if (!(is.na(well_twos.df$norm.activity[well_twos.df$grouping == y & well_twos.df$col == x]))[1] == TRUE) {
      temp <- nlm(well_gompertz_grouping, c(0.004, 0.14), well_twos.df, y, x)[["estimate"]]
      well_twos_params.df$Aparam[l] <- temp[1]
      well_twos_params.df$Gparam[l] <- temp[2]
    }
    l = l + 1
  }
}
...
for (x in 1:12) {
  for (y in 1:4) {
    well_twos_survivals.df[l] <- draw_gompertz(well_twos_params.df$Aparam[well_twos_params.df$col == x & well_twos_params.df$grouping == y], well_twos_params.df$Gparam[well_twos_params.df$col == x & well_twos_params.df$grouping == y])
    l <- l + 1
  }
}
...
well_threes_params.df <- data.frame(strain = rep(strains, each = 3), col = rep(1:12, each = 3), grouping = rep(1:3, nStrains), Aparam = rep(0, nStrains * 3), Gparam = rep(0, nStrains * 3))
...
for (x in 1:12) {
  for (y in 1:3) {
    if (!(is.na(well_threes.df$norm.activity[well_threes.df$grouping == y & well_threes.df$col == x]))[1] == TRUE) {
      temp <- nlm(well_gompertz_grouping, c(0.004, 0.14), well_threes.df, y, x)[["estimate"]]
      well_threes_params.df$Aparam[l] <- temp[1]
      well_threes_params.df$Gparam[l] <- temp[2]
    }
    l = l + 1
  }
}
...
for (x in 1:12) {
  for (y in 1:3) {
    well_threes_survivals.df[l] <- draw_gompertz(well_threes_params.df$Aparam[well_threes_params.df$col == x & well_threes_params.df$grouping == y], well_threes_params.df$Gparam[well_threes_params.df$col == x & well_threes_params.df$grouping == y])
    l <- l + 1
  }
}
...
well_fours_params.df <- data.frame(strain = rep(strains, each = 2), col = rep(1:12, each = 2), grouping = rep(1:2, nStrains), Aparam = rep(0, nStrains * 2), Gparam = rep(0, nStrains * 2))
...
for (x in 1:12) {
  for (y in 1:2) {
    if (!(is.na(well_fours.df$norm.activity[well_fours.df$grouping == y & well_fours.df$col == x]))[1] == TRUE) {
      temp <- nlm(well_gompertz_grouping, c(0.004, 0.14), well_fours.df, y, x)[["estimate"]]
      well_fours_params.df$Aparam[l] <- temp[1]
      well_fours_params.df$Gparam[l] <- temp[2]
    }
    l = l + 1
  }
}
...
for (x in 1:12) {
  for (y in 1:2) {
    well_fours_survivals.df[l] <- draw_gompertz(well_fours_params.df$Aparam[well_fours_params.df$col == x & well_fours_params.df$grouping == y], well_fours_params.df$Gparam[well_fours_params.df$col == x & well_fours_params.df$grouping == y])
    l <- l + 1
  }
}
...
strain_params.df <- data.frame(strain = unique(strains), Aparam = rep(0, length(unique(strains))), Gparam = rep(0, length(unique(strains))))

l = 1

for (x in unique(strains)) {
  if (!(is.na(strain.df$norm.activity[strain.df$strain == x]))[1] == TRUE) {
    temp <- nlm(strain_gompertz, c(0.004, 0.14), strain.df, x)[["estimate"]]
    strain_params.df$Aparam[l] <- temp[1]
    strain_params.df$Gparam[l] <- temp[2]
  }
  l = l + 1
}
...
for (x in unique(strain.df$strain)) {
    strain_survivals.df[l] <- draw_gompertz(strain_params.df$Aparam[strain_params.df$strain == x], strain_params.df$Gparam[strain_params.df$strain == x])
    l <- l + 1
}
...
for (i in 1:nrow(params.df)) {
  if (params.df$Aparam[i] == 0 & params.df$Gparam[i] == 0) {
    params.df$Aparam[i] <- NA
    params.df$Gparam[i] <- NA
  }
}

ggplot(params.df) + aes(x = Aparam, y = Gparam, size = grouping, color = grouping) + geom_point(alpha = 0.6) + facet_wrap( ~ strain) + presentation + big.legend + rotate.y.axis + labs(x = "A Parameter", y = "G Parameter")
...
heritabilities <- data.frame(parameter = rep(c("Aparam", "Gparam"), length(unique(params.df$grouping))), grouping = rep(unique(params.df$grouping), each = 2), herit = rep(0, length(unique(params.df$grouping)) * 2))
...
for (i in (1:length(unique(params.df$grouping))) * 2) {
  ...
  x <- params.df %>% filter(grouping == levels(params.df$grouping)[i / 2]) %>% select(c(strain, Gparam))
  x$pheno <- x$Gparam
  x <- x %>% select(c(strain, pheno))

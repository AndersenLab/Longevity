...
for (x in 1:12) {
  for (y in rows) {
    if (!(is.na(well.df$norm.activity[well.df$row == y & well.df$col == x]))[1] == TRUE) {
      temp <- nlm(well_gompertz, c(2, 1), well.df, y, x)[["estimate"]]
...
for (x in 1:12) {
  for (y in 1:4) {
    if (!(is.na(well_twos.df$norm.activity[well_twos.df$grouping == y & well_twos.df$col == x]))[1] == TRUE) {
      temp <- nlm(well_gompertz_grouping, c(2, 1), well_twos.df, y, x)[["estimate"]]
...
for (x in 1:12) {
  for (y in 1:3) {
    if (!(is.na(well_threes.df$norm.activity[well_threes.df$grouping == y & well_threes.df$col == x]))[1] == TRUE) {
      temp <- nlm(well_gompertz_grouping, c(2, 1), well_threes.df, y, x)[["estimate"]]
...
for (x in 1:12) {
  for (y in 1:2) {
    if (!(is.na(well_fours.df$norm.activity[well_fours.df$grouping == y & well_fours.df$col == x]))[1] == TRUE) {
      temp <- nlm(well_gompertz_grouping, c(2, 1), well_fours.df, y, x)[["estimate"]]
...
for (x in unique(strains)) {
  if (!(is.na(strain.df$norm.activity[strain.df$strain == x]))[1] == TRUE) {
    temp <- nlm(strain_gompertz, c(2, 1), strain.df, x)[["estimate"]]
...
ggplot(params.df) + aes(x = Aparam, y = Gparam, size = grouping, color = grouping) + geom_point(alpha = 0.6) + facet_wrap( ~ strain) + ylim(0, 25) + xlim(0, 4) + presentation + big.legend + rotate.y.axis + labs(x = "A Parameter", y = "G Parameter")

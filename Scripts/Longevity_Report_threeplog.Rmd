...
well_params.df <- data.frame(strain = rep(strains, each = length(rows)), col = rep(1:12, each = length(rows)), row = rep(rows, nStrains), aparam = rep(0, nStrains * replicates), bparam = rep(0, nStrains * replicates), kparam = rep(0, nStrains * replicates))
...
for (x in 1:12) {
  for (y in rows) {
    if (!(is.na(well.df$norm.activity[well.df$row == y & well.df$col == x]))[1] == TRUE) {
      temp <- nlm(well_threeplog, c(0.000004, 0.2, 0.4), well.df, y, x)[["estimate"]]
      well_params.df$aparam[l] <- temp[1]
      well_params.df$bparam[l] <- temp[2]
      well_params.df$kparam[l] <- temp[3]
    }
    l = l + 1
  }
}
...
ggplot(well.df) + geom_point(aes(x = day, y = norm.activity)) + geom_line(aes(x = day, y = yvalue, color = "red"), size = 2, alpha = 0.8) + facet_grid(row ~ col, labeller = label_strains) + ylim(0, 100) + presentation + remove.legend + rotate.y.axis + labs(x = "Day", y = "Normalized Activity")
...
well_twos_params.df <- data.frame(strain = rep(strains, each = 4), col = rep(1:12, each = 4), grouping = rep(1:4, nStrains), aparam = rep(0, nStrains * 4), bparam = rep(0, nStrains * 4), kparam = rep(0, nStrains * 4))
...
for (x in 1:12) {
  for (y in 1:4) {
    if (!(is.na(well_twos.df$norm.activity[well_twos.df$grouping == y & well_twos.df$col == x]))[1] == TRUE) {
      temp <- nlm(well_threeplog_grouping, c(0.000004, 0.2, 0.4), well_twos.df, y, x)[["estimate"]]
      well_twos_params.df$aparam[l] <- temp[1]
      well_twos_params.df$bparam[l] <- temp[2]
      well_twos_params.df$kparam[l] <- temp[3]
    }
    l = l + 1
  }
}
...
ggplot(well_twos.df) + geom_point(aes(x = day, y = norm.activity)) + geom_line(aes(x = day, y = yvalue, color = "red"), size = 2, alpha = 0.8) + facet_grid(grouping ~ col, labeller = label_strains) + ylim(0, 100) + presentation + remove.legend + rotate.y.axis + labs(x = "Day", y = "Normalized Activity")
...
well_threes_params.df <- data.frame(strain = rep(strains, each = 3), col = rep(1:12, each = 3), grouping = rep(1:3, nStrains), aparam = rep(0, nStrains * 3), bparam = rep(0, nStrains * 3), kparam = rep(0, nStrains * 3))
...
for (x in 1:12) {
  for (y in 1:3) {
    if (!(is.na(well_threes.df$norm.activity[well_threes.df$grouping == y & well_threes.df$col == x]))[1] == TRUE) {
      temp <- nlm(well_threeplog_grouping, c(0.000004, 0.2, 0.4), well_threes.df, y, x)[["estimate"]]
      well_threes_params.df$aparam[l] <- temp[1]
      well_threes_params.df$bparam[l] <- temp[2]
      well_threes_params.df$kparam[l] <- temp[3]
    }
    l = l + 1
  }
}
...
ggplot(well_threes.df) + geom_point(aes(x = day, y = norm.activity)) + geom_line(aes(x = day, y = yvalue, color = "red"), size = 2, alpha = 0.8) + facet_grid(grouping ~ col, labeller = label_strains) + ylim(0, 100) + presentation + remove.legend + rotate.y.axis + labs(x = "Day", y = "Normalized Activity")
...
well_fours_params.df <- data.frame(strain = rep(strains, each = 2), col = rep(1:12, each = 2), grouping = rep(1:2, nStrains), aparam = rep(0, nStrains * 2), bparam = rep(0, nStrains * 2), kparam = rep(0, nStrains * 2))
...
for (x in 1:12) {
  for (y in 1:2) {
    if (!(is.na(well_fours.df$norm.activity[well_fours.df$grouping == y & well_fours.df$col == x]))[1] == TRUE) {
      temp <- nlm(well_threeplog_grouping, c(0.000004, 0.2, 0.4), well_fours.df, y, x)[["estimate"]]
      well_fours_params.df$aparam[l] <- temp[1]
      well_fours_params.df$bparam[l] <- temp[2]
      well_fours_params.df$kparam[l] <- temp[3]
    }
    l = l + 1
  }
}
...
ggplot(well_fours.df) + geom_point(aes(x = day, y = norm.activity)) + geom_line(aes(x = day, y = yvalue, color = "red"), size = 2, alpha = 0.8) + facet_grid(grouping ~ col, labeller = label_strains) + ylim(0, 100) + presentation + remove.legend + rotate.y.axis + labs(x = "Day", y = "Normalized Activity")
...
strain_params.df <- data.frame(strain = unique(strains), aparam = rep(0, length(unique(strains))), bparam = rep(0, length(unique(strains))), kparam = rep(0, length(unique(strains))))

l = 1

for (x in unique(strains)) {
  if (!(is.na(strain.df$norm.activity[strain.df$strain == x]))[1] == TRUE) {
    temp <- nlm(strain_threeplog, c(0.000004, 0.2, 0.4), strain.df, x)[["estimate"]]
    strain_params.df$aparam[l] <- temp[1]
    strain_params.df$bparam[l] <- temp[2]
    strain_params.df$kparam[l] <- temp[3]
  }
  l = l + 1
}
...
ggplot(strain.df) + geom_point(aes(x = day, y = norm.activity)) + geom_line(aes(x = day, y = yvalue, color = "red"), size = 2, alpha = 0.8) + facet_wrap( ~ strain) + ylim(0, 100) + presentation + remove.legend + rotate.y.axis + labs(x = "Day", y = "Normalized Activity")
...
for (i in 1:nrow(params.df)) {
  if (params.df$aparam[i] == 0 & params.df$bparam[i] == 0 & params.df$kparam[i] == 0) {
...
ggplot(params.df) + aes(x = aparam, y = bparam, size = grouping, color = grouping) + geom_point(alpha = 0.6) + facet_wrap( ~ strain) + ylim(0, 25) + xlim(0, 4) + presentation + big.legend + rotate.y.axis + labs(x = "a Parameter", y = "b Parameter")
...
ggplot(plotting.df) + aes(x = as.numeric(grouping), y = herit, color = parameter, size = 2) + geom_line() + labs(x = "Number of Wells Grouped", y = "Heritability") + ylim(0, 1) + presentation + remove.legend + rotate.y.axis

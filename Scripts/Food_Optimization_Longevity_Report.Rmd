...
well_params.df <- data.frame(strain = rep(levels(factor(well.df$strain)), each = length(levels(factor(well.df$row)))), row = rep(levels(factor(well.df$row)), length(levels(factor(well.df$strain)))), bparam = rep(0, length(strains) * replicates), cparam = rep(0, length(strains) * replicates))
...
strain_params.df <- data.frame(strain = levels(factor(strain.df$strain)), bparam = rep(0, length(strains)), cparam = rep(0, length(strains)))

l = 1

for (x in levels(factor(strain.df$strain))) {

...
if (exists("temperature") == TRUE) { 
  cleaned.df$temp <- rep(0, length(cleaned.df$day))
  for (i in 1:length(cleaned.df$day)) {
    cleaned.df$temp[i] = temperature[cleaned.df$day[i]]
  }
  temp_normalized.df <- cleaned.df %>%
  group_by(strain, row, day, time) %>%
  mutate(temp.norm.act = activity / temp)
...
well.df <- cleaned.df %>%
  ...
  summarise(mean.activity = mean(activity), 
...
if (exists("temperature") == TRUE) {
  well.df$temp <- rep(temperature, nstrains * replicates)
  well.df <- well.df %>%
  mutate(temp.norm.act = total.activity / temp)
}
...
save(well.df, file = paste(data.loc, "/", experiment, "_Well.Rda", sep = ""), ascii = TRUE)
...
```{r Well_Adjusted, warning = FALSE, message = FALSE, echo = FALSE, fig.path = paste("Scripts/", data.loc, "/", experiment, "_", sep = ""), fig.width = 15, fig.height = 12}
...
for (x in levels(factor(well.df$strain))) {
  for (y in levels(factor(well.df$row))) {
    if (!(is.na(well.df$norm.act[well.df$row == y & well.df$strain == x]))[1] == TRUE) {
      temp <- nlm(well_twoplog, c(2, 2), well.df, y, x)[["estimate"]]
...
times <- 1:length(levels(well.df$day))
...
test <- data.frame(V1 = 1:12)
test$V2 <- c("A", "B", "C", "D", "E", "F", "G", "H", "I", "J", "K", "L")
...
#strain_test.df <- strain.df %>%
#  group_by(strain) %>%
#  mutate(max.sum = day[which.max(norm.act)[1]]) %>%
#  mutate(adjusted.activity = ifelse(day < max.sum, 100, norm.act))

#strain_test.df <- ungroup(strain_test.df)

#for (i in 1:length(strain_test.df$strain)) {
#  print(i)
#  print(ifelse(strain_test.df$day[i] < strain_test.df$max.sum[i], 100, strain_test.df$norm.act[i]))
#}

```

```{r Strain_Adjusted, warning = FALSE, message = FALSE, echo = FALSE, fig.path = paste("Scripts/", data.loc, "/", experiment, "_", sep = ""), fig.width = 15, fig.height = 12}
...
times <- 1:length(levels(strain.df$day))

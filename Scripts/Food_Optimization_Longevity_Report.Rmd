...
comparison_plot <- ggplot(summary.df) + geom_line(aes(x = day, y = norm.act)) + facet_wrap( ~ strain) +
  labs(x = "Time (day)", y = "Activity")
...
survivals.df <- data.frame(CB4856 = draw_twoplog(strain_params.df$bparam[strain_params.df$strain == "CB4856"], strain_params.df$cparam[strain_params.df$strain == "CB4856"]), CX11314 = draw_twoplog(strain_params.df$bparam[strain_params.df$strain == "CX11314"], strain_params.df$cparam[strain_params.df$strain == "CX11314"]), ED3017 = draw_twoplog(strain_params.df$bparam[strain_params.df$strain == "ED3017"], strain_params.df$cparam[strain_params.df$strain == "ED3017"]), JT11398 = draw_twoplog(strain_params.df$bparam[strain_params.df$strain == "JT11398"], strain_params.df$cparam[strain_params.df$strain == "JT11398"]), DL238 = draw_twoplog(strain_params.df$bparam[strain_params.df$strain == "DL238"], strain_params.df$cparam[strain_params.df$strain == "DL238"]), N2 = draw_twoplog(strain_params.df$bparam[strain_params.df$strain == "N2"], strain_params.df$cparam[strain_params.df$strain == "N2"]), MY23 = draw_twoplog(strain_params.df$bparam[strain_params.df$strain == "MY23"], strain_params.df$cparam[strain_params.df$strain == "MY23"]), JU258 = draw_twoplog(strain_params.df$bparam[strain_params.df$strain == "JU258"], strain_params.df$cparam[strain_params.df$strain == "JU258"]), EG4725 = draw_twoplog(strain_params.df$bparam[strain_params.df$strain == "EG4725"], strain_params.df$cparam[strain_params.df$strain == "EG4725"]), LKC34 = draw_twoplog(strain_params.df$bparam[strain_params.df$strain == "LKC34"], strain_params.df$cparam[strain_params.df$strain == "LKC34"]), JU775 = draw_twoplog(strain_params.df$bparam[strain_params.df$strain == "JU775"], strain_params.df$cparam[strain_params.df$strain == "JU775"]), MY16 = draw_twoplog(strain_params.df$bparam[strain_params.df$strain == "MY16"], strain_params.df$cparam[strain_params.df$strain == "MY16"])) %>% gather(strain, yvalue, 1:12)

fits.df <- summary.df
fits.df$yvalue <- survivals.df$yvalue


save(fits.df, file = paste(data.loc, "/", experiment, "_Fits.Rda", sep = ""), ascii = TRUE)

```

## 2 Parameter Logistic Function Fits by Strain

```{r Fits, warning = F, message = F, echo = F, fig.path = paste("Scripts/", data.loc, "/", experiment, "_", sep = ""), fig.width = 15, fig.height = 12}

ggplot(fits.df) + geom_line(aes(x = day, y = norm.act)) + ylim(0, 100) + geom_line(aes(x = day, y = yvalue, color = "red")) + facet_wrap( ~ strain)

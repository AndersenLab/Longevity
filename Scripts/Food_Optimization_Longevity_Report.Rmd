...
normalized.df <- cleaned.df %>%
  group_by(col, row) %>%
  mutate(norm.act = 100 * activity / max(activity, na.rm = T))

cleaned.df <- ungroup(cleaned.df)

save(normalized.df, file = paste(data.loc, "/", experiment, "_Normalized.Rda", sep = ""), ascii = TRUE)

```

## Normalized Data

```{r Normalized, warning = F, message = F, echo = F, fig.path = paste("Scripts/", data.loc, "/", experiment, "_", sep = ""), fig.width = 15, fig.height = 12}

ggplot(normalized.df) + aes(x = factor(day), y = norm.act) + ylim(0, 100) + geom_boxplot() + facet_grid(row ~ strain) +
...
ggplot(summary.df) + aes(x = day, y = norm.act, color = strain) + ylim(0, 100) + geom_line() +
  labs(x = "Time (day)", y = "Activity")

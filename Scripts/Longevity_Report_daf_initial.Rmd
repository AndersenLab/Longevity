...
raw.df <- ldply(inputList, generateDataFrameFromFile) %>% 
  filter(time == 2)
raw.df$day <- rep(pullDateInformation(files), each = nStrains * replicates)
...
ordered.df$strain <- rep(strains, each = nFiles * replicates)
ordered.df$uniqueStrain <- rep(uniqueStrains, each = nFiles * replicates)
ordered.df$num <- rep(nums, each = nFiles)
...
ggplot(heritabilities) + aes(x = factor(parameter), y = herit, color = grouping) + geom_point() + labs(x = "Parameter", y = "Heritability") + ylim(0, 1) + presentation

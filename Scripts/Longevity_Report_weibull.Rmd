...
well_params.df <- data.frame(strain = rep(strains, each = length(rows)), col = rep(1:12, each = length(rows)), row = rep(rows, nStrains), aparam = rep(0, nStrains * replicates), bparam = rep(0, nStrains * replicates))
...
for (x in 1:12) {
  for (y in rows) {
    if (!(is.na(well.df$norm.activity[well.df$row == y & well.df$col == x]))[1] == TRUE) {
      temp <- nlm(well_weibull, c(2, 10), well.df, y, x)[["estimate"]]
...
well_twos_params.df <- data.frame(strain = rep(strains, each = 4), col = rep(1:12, each = 4), grouping = rep(1:4, nStrains), aparam = rep(0, nStrains * 4), bparam = rep(0, nStrains * 4))
...
for (x in 1:12) {
  for (y in 1:4) {
    if (!(is.na(well_twos.df$norm.activity[well_twos.df$grouping == y & well_twos.df$col == x]))[1] == TRUE) {
      temp <- nlm(well_weibull_grouping, c(2, 10), well_twos.df, y, x)[["estimate"]]
...
well_threes_params.df <- data.frame(strain = rep(strains, each = 3), col = rep(1:12, each = 3), grouping = rep(1:3, nStrains), aparam = rep(0, nStrains * 3), bparam = rep(0, nStrains * 3))
...
for (x in 1:12) {
  for (y in 1:3) {
    if (!(is.na(well_threes.df$norm.activity[well_threes.df$grouping == y & well_threes.df$col == x]))[1] == TRUE) {
      temp <- nlm(well_weibull_grouping, c(2, 10), well_threes.df, y, x)[["estimate"]]
...
well_fours_params.df <- data.frame(strain = rep(strains, each = 2), col = rep(1:12, each = 2), grouping = rep(1:2, nStrains), aparam = rep(0, nStrains * 2), bparam = rep(0, nStrains * 2))
...
for (x in 1:12) {
  for (y in 1:2) {
    if (!(is.na(well_fours.df$norm.activity[well_fours.df$grouping == y & well_fours.df$col == x]))[1] == TRUE) {
      temp <- nlm(well_weibull_grouping, c(2, 10), well_fours.df, y, x)[["estimate"]]
...
strain_params.df <- data.frame(strain = unique(strains), aparam = rep(0, length(unique(strains))), bparam = rep(0, length(unique(strains))))

l = 1

for (x in unique(strains)) {
  if (!(is.na(strain.df$norm.activity[strain.df$strain == x]))[1] == TRUE) {
    temp <- nlm(strain_weibull, c(2, 10), strain.df, x)[["estimate"]]
...
for (i in 1:nrow(params.df)) {
  if (params.df$aparam[i] == 0 & params.df$bparam[i] == 0) {
    params.df$aparam[i] <- NA
    params.df$bparam[i] <- NA
  }
}

ggplot(params.df) + aes(x = aparam, y = bparam, size = grouping, color = grouping) + geom_point(alpha = 0.6) + facet_wrap( ~ strain) + presentation + big.legend + rotate.y.axis + labs(x = "a Parameter", y = "b Parameter")

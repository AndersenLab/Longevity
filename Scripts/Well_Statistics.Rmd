...
well_twos_params.df <- data.frame(strain = rep(strains, each = 4), grouping = rep(1:4, nStrains), bparam = rep(0, nStrains * 4), cparam = rep(0, nStrains * 4))
...
well_threes_params.df <- data.frame(strain = rep(strains, each = 3), grouping = rep(1:3, nStrains), bparam = rep(0, nStrains * 3), cparam = rep(0, nStrains * 3))
...
well_fours_params.df <- data.frame(strain = rep(strains, each = 2), grouping = rep(1:2, nStrains), bparam = rep(0, nStrains * 2), cparam = rep(0, nStrains * 2))
...
ggplot(params.df) + aes(x = log(bparam), y = log(cparam), size = grouping, color = strain) + geom_point(alpha = 0.5) + facet_wrap( ~ strain)

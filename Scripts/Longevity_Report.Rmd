...
ordered.df$strain <- rep(strains, each = nFiles * replicates * fileAttributes.df$numBins[1])
...
ggplot(ordered.df) + aes(x = day, y = activity) + geom_point() + facet_grid(row ~ col, labeller = label_strains) +
...
ggplot(cleaned.df) + aes(x = factor(day), y = activity) + geom_boxplot() + facet_grid(row ~ col, labeller = label_strains) +
  labs(x= "Time (day)", y = "Raw Activity")
...
ggplot(well.df) + geom_line(aes(x = day, y = mean.activity)) + 
  ...
  facet_grid(row ~ col, labeller = label_strains) + labs(x = "Time (day)", y = "Mean Activity")
...
well_params.df <- data.frame(strain = rep(strains, each = length(rows)), row = rep(rows, nStrains), bparam = rep(0, nStrains * replicates), cparam = rep(0, nStrains * replicates))
...
ggplot(well.df) + geom_line(aes(x = day, y = norm.act)) + ylim(0, 100) + geom_line(aes(x = day, y = yvalue, color = "red")) + facet_grid(row ~ col, labeller = label_strains)

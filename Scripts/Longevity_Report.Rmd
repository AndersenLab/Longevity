...
well_params.df <- data.frame(strain = rep(strains, each = length(rows)), col = rep(1:12, each = length(rows)), row = rep(rows, nStrains), bparam = 0, cparam = 0, minimum = 0)
...
for (x in 1:12) {
  for (y in rows) {
    if (!(is.na(well.df$norm.activity[well.df$row == y & well.df$col == x]))[1] == TRUE) {
      temp <- nlm(well_twoplog, c(2, 10), well.df, y, x)
      well_params.df$bparam[l] <- temp[["estimate"]][1]
      well_params.df$cparam[l] <- temp[["estimate"]][2]
...
well_twos_params.df <- data.frame(strain = rep(strains, each = 4), col = rep(1:12, each = 4), grouping = rep(1:4, nStrains), bparam = 0, cparam = 0, minimum = 0)
...
for (x in 1:12) {
  for (y in 1:4) {
    if (!(is.na(well_twos.df$norm.activity[well_twos.df$grouping == y & well_twos.df$col == x]))[1] == TRUE) {
      temp <- nlm(well_twoplog_grouping, c(2, 10), well_twos.df, y, x)
      well_twos_params.df$bparam[l] <- temp[["estimate"]][1]
      well_twos_params.df$cparam[l] <- temp[["estimate"]][2]
...
well_threes_params.df <- data.frame(strain = rep(strains, each = 3), col = rep(1:12, each = 3), grouping = rep(1:3, nStrains), bparam = 0, cparam = 0, minimum = 0)
...
for (x in 1:12) {
  for (y in 1:3) {
    if (!(is.na(well_threes.df$norm.activity[well_threes.df$grouping == y & well_threes.df$col == x]))[1] == TRUE) {
      temp <- nlm(well_twoplog_grouping, c(2, 10), well_threes.df, y, x)
      well_threes_params.df$bparam[l] <- temp[["estimate"]][1]
      well_threes_params.df$cparam[l] <- temp[["estimate"]][2]
...
well_fours_params.df <- data.frame(strain = rep(strains, each = 2), col = rep(1:12, each = 2), grouping = rep(1:2, nStrains), bparam = 0, cparam = 0, minimum = 0)
...
for (x in 1:12) {
  for (y in 1:2) {
    if (!(is.na(well_fours.df$norm.activity[well_fours.df$grouping == y & well_fours.df$col == x]))[1] == TRUE) {
      temp <- nlm(well_twoplog_grouping, c(2, 10), well_fours.df, y, x)
      well_fours_params.df$bparam[l] <- temp[["estimate"]][1]
      well_fours_params.df$cparam[l] <- temp[["estimate"]][2]
...
strain_params.df <- data.frame(strain = unique(strains), bparam = 0, cparam = 0, minimum = 0)

l = 1

for (x in unique(strains)) {
  if (!(is.na(strain.df$norm.activity[strain.df$strain == x]))[1] == TRUE) {
    temp <- nlm(strain_twoplog, c(2, 10), strain.df, x)
    strain_params.df$bparam[l] <- temp[["estimate"]][1]
    strain_params.df$cparam[l] <- temp[["estimate"]][2]
...
for (i in 1:nrow(params.df)) {
  if (params.df$bparam[i] == 0 & params.df$cparam[i] == 0 & params.df$minimum[i] == 0) {
...
ggplot(params.df) + aes(x = bparam, y = cparam, size = grouping, color = grouping) + geom_point(alpha = 0.6) + facet_wrap( ~ strain) + ylim(0, 25) + xlim(0, 4) + presentation + big.legend + rotate.y.axis + labs(x = "b Parameter", y = "c Parameter")
...
ggplot(minimum.df) + aes(x = grouping, y = minimum.avg, size = 2) + geom_line() + labs(x = "Number of Wells Grouped", y = "Average Least Squares Deviation") + presentation + remove.legend + rotate.y.axis

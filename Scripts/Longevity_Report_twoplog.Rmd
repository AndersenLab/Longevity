...
experimentName <- paste(experimentName, "_TwoPLog", sep = "")
...
ggplot(params.df) + aes(x = bparam, y = cparam, size = grouping, color = grouping) + geom_point(alpha = 0.6) + facet_wrap( ~ strain) + ylim(0, 25) + xlim(0, 4) + presentation + big.legend + labs(x = "b Parameter", y = "c Parameter")
...
ggplot(well_params.df) + aes(x = strain, y = bparam) + geom_boxplot() + geom_jitter() + presentation
...
minimum.df$grouping <- c(1, 2, 3, 4, 5)
